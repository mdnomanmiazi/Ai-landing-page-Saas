...
- The design must feel premium, modern, and visually impressive.
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        // 2. Use the user-selected model (Default fallback to gpt-5)
        model: model || "gpt-5", 
        messages: [
          { role: "system", content: systemPrompt },
          { role: "user", content: prompt }
        ]
        // Note: Removing max_tokens/temperature parameters to ensure compatibility with all model types (Codex/Reasoning)
      }),
    });
