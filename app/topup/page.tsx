...
export default function TopUpPage() {
  ...
  const [amountUSD, setAmountUSD] = useState(10); // Default $10
  const [user, setUser] = useState<any>(null);
  const RATE = 125; // 1 USD = 125 BDT
  ...
  const handleTopUp = async () => {
    ...
    // We store the USD amount so we credit the correct value on success
    localStorage.setItem('pending_topup', amountUSD.toString());
    
    // Calculate BDT for the gateway
    const amountBDT = Math.ceil(amountUSD * RATE);

    await processPayment(
      [{
        name: "Wallet Deposit (USD)",
        price: amountBDT, // Gateway processes BDT
        quantity: 1,
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-md mx-auto mt-16 p-8 bg-white rounded-2xl shadow-xl border border-slate-100">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-900">Add Funds (USD)</h1>
            <p className="text-sm text-slate-500">Min $5 • Rate: 1 USD = {RATE} BDT</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {[5, 10, 20, 50].map((val) => (
            <button
              ...
              className={`py-3 rounded-lg border font-bold transition-all ${amountUSD === val ? 'border-blue-600 bg-blue-50 text-blue-700 ring-1 ring-blue-600' : 'border-slate-200 hover:border-blue-300'}`}
            >
              ${val}
            </button>
          ))}
        ...
        <div className="mb-8">
          ...
          <div className="mt-3 flex justify-between items-center text-sm bg-slate-50 p-3 rounded-lg border border-slate-200">
            <span className="text-slate-500">You pay in BDT:</span>
            <span className="font-mono font-bold text-slate-900 text-lg">৳ {(amountUSD * RATE).toLocaleString()}</span>
          </div>
        </div>

        <button 
          onClick={handleTopUp}
          disabled={isLoading || !user || amountUSD < 5}
        ...
        >
          {isLoading ? "Processing..." : `Pay ৳ ${(amountUSD * RATE).toLocaleString()}`}
